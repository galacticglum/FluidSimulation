...
void ImGuiSystem::MouseButtonCallback(GLFWwindow*, int button, int action, int /*mods*/)
{
	if (action == GLFW_PRESS && button >= 0 && button < 3)
		g_MouseJustPressed[button] = true;
}

void ImGuiSystem::ScrollCallback(GLFWwindow*, double /*xoffset*/, double yoffset)
{
	g_MouseWheel += (float)yoffset; // Use fractional mouse wheel.
}

void ImGuiSystem::KeyCallback(GLFWwindow*, int key, int, int action, int mods)
{
	ImGuiIO& io = ImGui::GetIO();
...
void ImGuiSystem::CharCallback(GLFWwindow*, unsigned int c)
{
	ImGuiIO& io = ImGui::GetIO();
	if (c > 0 && c < 0x10000)
...
bool ImGuiSystem::CreateDeviceObjects()
{
	// Backup GL state
...
bool ImGuiSystem::Init(GLFWwindow* window, bool install_callbacks)
{
	g_Window = window;
